...
int main() {
    ...
    while(cases--) {
        ...
        Linea firstLine = Linea(Punto(p11,p12),Punto(p21,p22));
        Linea secondLine = Linea(Punto(p31,p32),Punto(p41,p42));
        int i = InterseccionSegmentos(firstLine, secondLine);
        if (i == 1) {
            Punto p = PuntoInterseccion(firstLine, secondLine);
            cout<<"POINT "<<fixed<<setprecision(2)<<p.x<<" "<<p.y<<endl;
        } else if (i == -1) {
            cout<<"LINE"<<endl;
        } else {
            if (LineasIguales(firstLine, secondLine)) {
                cout<<"LINE"<<endl;
            } else {
                cout<<"NONE"<<endl;
            }
